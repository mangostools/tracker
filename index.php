               ...
               elseif (is_numeric($areasort)) {
                   ...
                   for ($i = 0; $i <= 8; $i++) {
                       if ($i != 8) {
                           if ($row["RequiredRaces"] & 1 << $i)
                               echo strtolower("<img src=\"images/race_" . str_replace(" ", "", mysql_result(mysql_query("SELECT name FROM $udbviewdb.chrraces WHERE id=" . ($i + 1)), 0)) . "_male.jpg\" style=width:32px> ");
                           else
                               echo strtolower("<img src=\"images/race_" . str_replace(" ", "", mysql_result(mysql_query("SELECT name FROM $udbviewdb.chrraces WHERE id=" . ($i + 1)), 0)) . "_male.jpg\" style=width:32px;opacity:0.3> ");
                       }
                   }
                   echo "</br>";
                   if ($row["RequiredMinRepFaction"] or $row["RequiredMinRepFaction"]) {
                       ...
                       if ($row["RequiredMinRepFaction"])
                           echo mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=" . $row["RequiredMinRepFaction"]), 0) . " > " . $row["RequiredMinRepValue"] . "; ";
                       if ($row["RequiredMaxRepFaction"])
                           echo mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=" . $row["RequiredMaxRepFaction"]), 0) . " < " . $row["RequiredMaxRepValue"];
                       echo "</br>";
                   }
                   ...
                   if ($row["SrcSpell"] > 0)
                       $temp.="<tr><td><b>Spell:</b> " . mysql_result(mysql_query("SELECT name FROM $udbviewdb.spell WHERE id=" . $row["SrcSpell"]), 0) . "</td></tr>";
                   if (!empty($temp)) {
                   ...
                   if ($row["RepObjectiveFaction"] > 0 && $row["RepObjectiveValue"] > 0)
                       $temp.="<tr><td><b>Gain Reputation:</b> " . mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=" . $row["RepObjectiveFaction"]), 0) . " => " . $row["RepObjectiveValue"] . "</td></tr>";
                   ...
                   if ($row["CharTitleId"] > 0)
                       $temp.="<tr><td><b>Title:</b> " . mysql_result(mysql_query("SELECT name FROM $udbviewdb.chartitles WHERE id=" . $row["CharTitleId"]), 0) . "</td></tr>";
