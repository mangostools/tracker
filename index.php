...
global $udbviewdb;
if($id>0)
  return mysql_result(mysql_query("SELECT name FROM $udbviewdb.users WHERE id=$id"),0);
else
  return "Unknown";
}
...
$zos_profession="-24,-101,-121,-181,-182,-201,-264,-304,-324,-762,-371,-373";
$zos_class="-61,-81,-82,-141,-161,-162,-261,-262,-263,-372";

$quest_flags=Array( "QUEST_FLAGS_NONE",		//0
		    ...
		    "QUEST_FLAGS_TBC_RACES",	//2048
		    "QUEST_FLAGS_DAILY",	//4096
		    "QUEST_FLAGS_FLAGS_PVP",//8192
            "QUEST_FLAGS_UNK4",//16384
            "QUEST_FLAGS_WEEKLY",//32768
            "QUEST_FLAGS_AUTOCOMPLETE",//65536
            "QUEST_FLAGS_UNK5",//131072
            "QUEST_FLAGS_UNK6",//262144
            "QUEST_FLAGS_AUTO_ACCEPT");	//524288

$special_flags=Array( "NONE",			//0
		      "REPEATABLE",		//1
		      "REQUIRES_TRIGGER");	//2

$problems=Array("No Problem",//0
                "No Quest Start",//1
                "No Quest End" //2
                );

$status=Array(	"Unknown",//0
		"not completable",//1
		"core issue",     //2
		"script issue",   //3
		"DB issue",       //4
...
$udbversion=Array("398","397","396","395","394","393","392");
...
if(isset($_GET["deletepost"]) && is_numeric($_GET["deletepost"]))
  {
  $u=mysql_result(mysql_query("SELECT user FROM $udbviewdb.status WHERE id =".$_GET["deletepost"]),0);
  if($u==$_SESSION["id"])
...
if(isset($_GET["doreg"]))
  ...
  if(!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && strlen($_POST["username"])>3 &&strlen($_POST["password"])>3 && $_POST["password"]==$_POST["password2"])
    {
    mysql_query("INSERT INTO $udbviewdb.users (`name`, `password`, `power`) VALUES (\"".mysql_real_escape_string($_POST["username"])."\", \"".md5($_POST["password"])."\", 1)") or die(mysql_error());
    $_GET["dologin"]=1;
    }
  }
if(isset($_GET["reg"]))
...
if(isset($_GET["dologin"]))
  ...
  if(isset($_POST["username"]) && isset($_POST["password"]))
    {
    $salt=time();
    if(mysql_result(mysql_query("SELECT COUNT(id) FROM $udbviewdb.users WHERE name=\"".mysql_real_escape_string($_POST["username"])."\" AND password=\"".md5($_POST["password"])."\""),0)==1)
      {
      $_SESSION["id"]=mysql_result(mysql_query("SELECT id FROM $udbviewdb.users WHERE name=\"".mysql_real_escape_string($_POST["username"])."\" AND password=\"".md5($_POST["password"])."\""),0);
      $_SESSION["hash"]=md5(md5($_POST["password"]).$salt);
      mysql_query("UPDATE $udbviewdb.users SET lastlogin=".$salt." WHERE id =".$_SESSION["id"]);
      $login = "Success";
      }
    else
      {
...
if(isset($_SESSION["id"]) && $_SESSION["id"]!=0)
  {
  $sql=mysql_query("SELECT COUNT(id) FROM $udbviewdb.users WHERE id=".$_SESSION["id"]." AND MD5(CONCAT( `password` , `lastlogin` )) = \"".$_SESSION["hash"]."\"") or die(mysql_error());
    if(mysql_result($sql,0)==1)
      $login="Logged in as ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.users WHERE id=".$_SESSION["id"]),0)." (<a href=index.php?dologout>Log Out</a>)";
    else
      unset($_SESSION);
  }
?>
...
for($i=0;$i<count($udbversion);$i++)
  $select_rev.="<option value=".$udbversion[$i]." ".($udbversion[$i]==$show_data_for_rev?"selected=selected":"").">".$udbversion[$i]."</option>";
...
if(is_numeric($query))
  ...
  if ($count==0)
    ...
    while($row=mysql_fetch_assoc($sql))
      {
      $res2=mysql_query("SELECT status, udbver FROM $udbviewdb.status WHERE quest_id = ".$row["entry"]." AND udbver>=".$udbversion[$c_udbversion]." GROUP BY status ASC, udbver DESC");
      $queststatus="";
      while($row2=mysql_fetch_array($res2))
        $queststatus.="<span style=color:".$statuscolor[$row2["status"]].">".$status[$row2["status"]]." in ".$row2["udbver"]."</span>; ";
      if(empty($queststatus))
        $queststatus=$status[0]." in ".$udbversion[0];
...
{
  $count=mysql_result(mysql_query("SELECT COUNT(entry) FROM $udbviewdb.problems WHERE entry NOT IN (SELECT quest_id FROM $udbviewdb.status)"),0);
  ...
  $sql=mysql_query("SELECT entry, problem FROM $udbviewdb.problems WHERE entry NOT IN (SELECT quest_id FROM $udbviewdb.status) ORDER BY entry ASC LIMIT $offset, 50");
  while($row=mysql_fetch_array($sql))
  {
    ...
    for($i=0;$i<count($problems);$i++)
      if($row["problem"]&1<<$i)echo $problems[$i+1]."; ";
    echo "</td>";
    echo "</tr>";
  }
...
elseif(isset($_GET["recent"]))
{
  ...
  if($offset=="x")
    $offset=0;
  $udbver = $show_data_for_rev=="any"?"":" AND udbver = ".$show_data_for_rev;
  $filter = $filter_status=="any"?"":" AND status = ".$filter_status;
  $count=mysql_result(mysql_query("SELECT COUNT(id) FROM $udbviewdb.status WHERE 1 $udbver $filter"),0);
  ...
  $sql=mysql_query("SELECT quest_id, udbver, user, report, status, ts FROM $udbviewdb.status WHERE 1 $udbver $filter ORDER BY ts DESC LIMIT $offset, 50");
  while($row=mysql_fetch_array($sql))
  {
    echo "<tr>";
    ...
    echo "<td>".mysql_result(mysql_query("SELECT name FROM $udbviewdb.users WHERE id =".$row["user"]),0)."</td>";
    echo "<td><span style=color:".$statuscolor[$row["status"]].">".$status[$row["status"]]."</span> in ".$row["udbver"]."</td>";
...
$unknown=$anz-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
$temp="";
$working=array();
for($i=1;$i<count($status);$i++)
  {
  $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
  if($working[$i] !=0)
...
$sql="SELECT count( mq.entry ) as anz , ua.map, um.name, um.type FROM $mangosdb.quest_template AS mq, $udbviewdb.areatable AS ua, $udbviewdb.map AS um WHERE mq.ZoneOrSort >0 AND mq.ZoneOrSort = ua.id AND ua.map = um.id GROUP BY um.id ASC ORDER BY um.type, ua.map ASC";
$type=-1;
...
while($row=mysql_fetch_assoc($result))
{
  if($type!=$row["type"])
  {
    ...
    switch($type)
    {
      case 0:echo "<tr><td colspan=4 style=background-color:#eee>Map</td></tr>";break;
      case 1:echo "<tr><td colspan=4 style=background-color:#eee>Instance</td></tr>";break;
      case 2:echo "<tr><td colspan=4 style=background-color:#eee>Raid</td></tr>";break;
      case 3:echo "<tr><td colspan=4 style=background-color:#eee>BG</td></tr>";break;
      default:break;
    }
  ...
  {
    $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (SELECT id FROM $udbviewdb.areatable WHERE map = ".$row["map"].")) AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working !=0)
  ...
  {
  $temp="";
  $unknown=$row["anz"]-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (SELECT id FROM $udbviewdb.areatable WHERE map = ".$row["map"].")) AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
  $working=array();
  for($i=1;$i<count($status);$i++)
    {
    $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (SELECT id FROM $udbviewdb.areatable WHERE map = ".$row["map"].")) AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working[$i] !=0)
...
echo "<tr><td>Name</td><td>Quests total</td><td>Trac Progress (<font color=black>unk</font>/<font color=red>bug</font>/<font color=brown>core</font>/<font color=orange>script</font>/<font color=darkcyan>DB</font>/<font color=green>ok</font>/<font color=blue>blizzlike</font>)</td><td>working</td></tr>";
  ...
  if(is_numeric($filter_status))
  {
    $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_profession.")) AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working !=0)
  ...
  {
    $temp="";
    $unknown=$profession-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_profession.")) AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
    $working=array();
    for($i=1;$i<count($status);$i++)
      {
        $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_profession.")) AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
        if($working[$i] !=0)
          $temp .="/<font color=".$statuscolor[$i].">".$working[$i]."</font>";
      }
    $temp ="<font color=".$statuscolor[0].">".$unknown."</font>".$temp; 
  ...
  if(is_numeric($filter_status))
  {
    $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_class.")) AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working !=0)
  ...
  {
    $temp="";
    $unknown=$class-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_class.")) AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
    $working=array();
    for($i=1;$i<count($status);$i++)
      {
      $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort IN (".$zos_class.")) AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
      if($working[$i] !=0)
        $temp .="/<font color=".$statuscolor[$i].">".$working[$i]."</font>";
      }
    $temp ="<font color=".$statuscolor[0].">".$unknown."</font>".$temp; 
  ...
  if(is_numeric($filter_status))
  {
    $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort <0 AND ZoneOrSort NOT IN (".$zos_class.",".$zos_profession.")) AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working !=0)
  ...
  {
    $temp="";
    $unknown=$event-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort <0 AND ZoneOrSort NOT IN (".$zos_class.",".$zos_profession.")) AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
    $working=array();
    for($i=1;$i<count($status);$i++)
      {
      $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort <0 AND ZoneOrSort NOT IN (".$zos_class.",".$zos_profession.")) AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
      if($working[$i] !=0)
        $temp .="/<font color=".$statuscolor[$i].">".$working[$i]."</font>";
      }
    $temp ="<font color=".$statuscolor[0].">".$unknown."</font>".$temp; 
  ...
  if(is_numeric($filter_status))
  {
    $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =0) AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
    if($working !=0)
  ...
  {
    $temp="";
    $unknown=$other-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =0) AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);;
    $working=array();
    for($i=1;$i<count($status);$i++)
      {
      $working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =0) AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
      if($working[$i] !=0)
        $temp .="/<font color=".$statuscolor[$i].">".$working[$i]."</font>";
      }
    $temp ="<font color=".$statuscolor[0].">".$unknown."</font>".$temp;   
...
if($areasort=="x" && $quest == "x" && is_numeric($map) && mysql_result(mysql_query("SELECT COUNT(id) FROM $udbviewdb.areatable WHERE map = $map"),0) == 1)
  $areasort = mysql_result(mysql_query("SELECT id FROM $udbviewdb.areatable WHERE map = $map"),0);

if($areasort==="x" && $quest == "x")
{
  if(is_numeric($map))
  { 
    $mapname = mysql_result(mysql_query("SELECT name FROM $udbviewdb.map WHERE id = $map"),0);
    ...
    $result=mysql_query("SELECT COUNT(m.entry) AS anz, m.ZoneOrSort,u.id, u.name FROM $mangosdb.quest_template as m, $udbviewdb.areatable as u WHERE u.map=$map AND u.id = m.ZoneOrSort GROUP BY m.ZoneOrSort ASC") or die(mysql_error());
  }
  elseif($map=="p")
  { 
    ...
    //professions
    $result=mysql_query("SELECT COUNT(m.entry) AS anz, m.ZoneOrSort,u.id, u.name FROM $mangosdb.quest_template as m, $udbviewdb.questsort as u WHERE u.id = -1*m.ZoneOrSort AND m.ZoneOrSort IN (-24,-101,-121,-181,-182,-201,-264,-304,-324,-762,-371,-373) GROUP BY m.ZoneOrSort ASC") or die(mysql_error());
  }
  elseif($map=="c")
  { 
    ...
    $result=mysql_query("SELECT COUNT(m.entry) AS anz, m.ZoneOrSort,u.id, u.name FROM $mangosdb.quest_template as m, $udbviewdb.questsort as u WHERE u.id = -1*m.ZoneOrSort AND m.ZoneOrSort IN (-61, -81, -82,-141,-161 , -162 ,-261 , -262 ,-263 , -372) GROUP BY m.ZoneOrSort ASC") or die(mysql_error());
  }
  elseif($map=="e")
  { 
  ...
  while($row=mysql_fetch_assoc($result))
  {
	if(is_numeric($filter_status))
	{
	  $working=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =".$row["ZoneOrSort"].") AND status = $filter_status AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
	  if($working !=0)
	...
	{
	  $temp="";
	  $unknown=$row["anz"]-mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =".$row["ZoneOrSort"].") AND status > 0 AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
	  $working=Array();
	  for($i=1;$i<count($status);$i++)
	  {
		$working[$i]=mysql_result(mysql_query("SELECT COUNT(DISTINCT quest_id) FROM $udbviewdb.status WHERE quest_id in (SELECT entry FROM $mangosdb.quest_template WHERE ZoneOrSort =".$row["ZoneOrSort"].") AND status = $i AND (udbver=".(is_numeric($show_data_for_rev)?$show_data_for_rev:"0 or udbver>0").")"),0);
		if($working[$i] !=0)
		  $temp .="/<font color=".$statuscolor[$i].">".$working[$i]."</font>";
	  }
	  $temp ="<font color=".$statuscolor[0].">".$unknown."</font>".$temp; 
...
elseif(is_numeric($areasort))
{
  switch($map)
  {
    case "p":echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&map=p>Professions</a>&nbsp;>>&nbsp;";break;
    case "c":echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&map=c>Classes</a>&nbsp;>>&nbsp;";break;
    case "e":echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&map=e>Events</a>&nbsp;>>&nbsp;";break;
    case "u":echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&map=u>Unknown</a>&nbsp;>>&nbsp;";break;
    default:
    {
      if($areasort>0)
      {
        $res=mysql_query("SELECT m.id, m.name FROM $udbviewdb.map as m,$udbviewdb.areatable as a WHERE m.id = a.map AND a.id=$areasort");
  ...
  if($areasort>0)
  {
    $mapname = mysql_result(mysql_query("SELECT a.name FROM $udbviewdb.areatable as a WHERE a.id=$areasort"),0);
    echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&areasort=$areasort>$mapname</a>&nbsp;>>&nbsp;";
  }
  if($areasort<0)
  {
    $mapname = mysql_result(mysql_query("SELECT a.name FROM $udbviewdb.questsort as a WHERE a.id=-1*$areasort"),0);
    echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&areasort=$areasort>$mapname</a>&nbsp;>>&nbsp;";
  }
  ...
  while($row=mysql_fetch_assoc($result))
  {
    ...
    else
    {
      $queststatus="&nbsp;";
      while($row2=mysql_fetch_array($res2))
        $queststatus.="<span class=\"tag".$row2["status"]."\" title=\"".$status[$row2["status"]]."\">".$row2["udbver"]."</span> ";
      echo "<tr><td><a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&quest=".$row["entry"].">".$row["entry"]."</a></td><td><a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&quest=".$row["entry"].">".$row["Title"]."</a></td><td>".$queststatus."</td></tr>";
    }
  }
}
elseif(is_numeric($quest))
{
    ...
    switch($areasort){
      case 0:echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&map=u>Unknown</a>&nbsp;>>&nbsp;";break;
	...
	echo "<a href=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&areasort=$areasort>$mapname</a>&nbsp;>>&nbsp;";
      ...
      echo "<b>Title:</b> ".htmlentities($row["Title"])." <a href=\"http://www.wowhead.com/?quest=".$quest."\" target=_blank>Wowhead</a> - <a href=\"http://thottbot.com/q".$quest."\" target=_blank>Thottbot</a> - <a href=\"http://wow.allakhazam.com/db/quest.html?wquest=$quest\" target=_blank>Allakhazam</a></br>";
      $questtype=$row["Type"]==0?"none":mysql_result(mysql_query("SELECT name FROM $udbviewdb.questinfo WHERE id=".$row["Type"]),0);
      echo "<b>Quest Type:</b> ".$questtype."</br>";
      if($row["SuggestedPlayers"]>0)echo "Suggested Players: ".$row["SuggestedPlayers"]."</br>";
	...
	echo mysql_result(mysql_query("SELECT name FROM $mangosdb.creature_template WHERE entry=".$creature_id),0)." (NPC) <a href=\"http://www.wowhead.com/?npc=".$creature_id."\" target=_blank>Wowhead</a> - <a href=\"http://thottbot.com/c".$creature_id."\" target=_blank>Thottbot</a> - <a href=\"http://wow.allakhazam.com/db/mob.html?wmob=$creature_id\" target=_blank>Allakhazam</a>";
      }
	...
	echo "<b>Point of Interest:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.map WHERE id=".$row["PointMapId"]),0)." (X:".$row["PointX"]." - Y:".$row["PointY"].")</br>";
      echo "</fieldset>";
	...
	{
	$skill=mysql_result(mysql_query("SELECT name FROM $udbviewdb.skillline WHERE id=".$row["SkillOrClass"]),0);
	echo "<b>Required Skill:</b> ".$skill." ".$row["RequiredSkillValue"]."</br>";
	}
      if($row["SkillOrClass"]<0)
	{
	$skill=mysql_result(mysql_query("SELECT name FROM $udbviewdb.chrclasses WHERE id=-1*".$row["SkillOrClass"]),0);
	echo "<b>Required Class:</b> ".$skill."</br>";
	}
      echo "<b>Required Races:</b> ";
      if($row["RequiredRaces"]==0)$row["RequiredRaces"]=1791;
      for($i=0;$i<=10;$i++)
	{
	  if($i != 8)
	  {
	    if($row["RequiredRaces"]&1<<$i)echo strtolower("<img src=\"images/race_".str_replace(" ","",mysql_result(mysql_query("SELECT name FROM $udbviewdb.chrraces WHERE id=".($i+1)),0))."_male.jpg\" style=width:32px> ");
	    else echo strtolower("<img src=\"images/race_".str_replace(" ","",mysql_result(mysql_query("SELECT name FROM $udbviewdb.chrraces WHERE id=".($i+1)),0))."_male.jpg\" style=width:32px;opacity:0.3> ");
          }
	}
      echo "</br>";
	...
	if($row["RequiredMinRepFaction"])echo mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=".$row["RequiredMinRepFaction"]),0)." > ".$row["RequiredMinRepValue"]."; ";
	if($row["RequiredMaxRepFaction"])echo mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=".$row["RequiredMaxRepFaction"]),0)." < ".$row["RequiredMaxRepValue"];
	echo "</br>";
	}
	...
	for($i=0;$i<=15;$i++)
	  if($row["QuestFlags"]&1<<$i)echo (1<<$i)."=".$quest_flags[$i+1]."; ";
	echo "</br>";
	}
	...
	for($i=0;$i<=2;$i++)
	  if($row["SpecialFlags"]&1<<$i)echo (1<<$i)."=".$special_flags[$i+1]."; ";
	echo "</br>";
	}
      ...
      //On Quest Start
      $temp="";
      if($row["SrcItemId"]>0 && $row["SrcItemCount"]>0)$temp.="<tr><td><b>Provided Item:</b> ".$row["SrcItemCount"]."x ".mysql_result(mysql_query("SELECT name FROM $mangosdb.item_template WHERE entry=".$row["SrcItemId"]),0)."</td></tr>";
      if($row["SrcSpell"]>0)$temp.="<tr><td><b>Spell:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.spell WHERE id=".$row["SrcSpell"]),0)."</td></tr>";
      if(!empty($temp))
	{
	...
	echo "</table></fieldset>";
      ...
      if(!empty($row["Details"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>Detail text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["Details"])."</i></p>";
      if(!empty($row["Objectives"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>Objectives text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["Objectives"])."</i></p>";
      if(!empty($row["RequestItemsText"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>Progress text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["RequestItemsText"])."</i></p>";
      if(!empty($row["OfferRewardText"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>Completion text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["OfferRewardText"])."</i></p>";
      if(!empty($row["EndText"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>End Text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["EndText"])."</i></p>";
      if(!empty($row["CompletedText"]))echo "<p style=\"text-indent:-6em;margin-left:6em\"><b>Completed Text:</b> <i>".str_ireplace(array("\$b","\$n","\$r","\$c","\$g"),array("</br>","&lt;Name&gt;","&lt;Race&gt;","&lt;Class&gt;","&lt;Custom&gt;"),$row["CompletedText"])."</i></p>";     
 echo "</fieldset>";
	...
	if(!empty($row["ReqItemId".$i]) && !empty($row["ReqItemCount".$i]))
	  {
	  $temp.="<tr><td>".$row["ReqItemCount".$i]."x ".mysql_result(mysql_query("SELECT name FROM $mangosdb.item_template WHERE entry=".$row["ReqItemId".$i]),0)."</td></tr>";
	  }
	...
	if(!empty($row["ReqSpellCast".$i]))
	  {
	  $temp .= "<tr><td>".mysql_result(mysql_query("SELECT name FROM $udbviewdb.spell WHERE id=".$row["ReqSpellCast".$i]),0)."</td></tr>";
	  }
	...
	echo "</table></fieldset></td>";
      ...
      //Other
      $temp="";
      if($row["LimitTime"]>0)$temp.="<tr><td><b>Time Limit:</b> ".floor($row["LimitTime"]/60).":".($row["LimitTime"]%60<10?"0":"").($row["LimitTime"]%60)."</td></tr>";
      if($row["RewOrReqMoney"]<0)$temp.="<tr><td><b>Money:</b> ".(-$row["RewOrReqMoney"]>=10000?floor(-$row["RewOrReqMoney"]/10000)."g":"")
							 .((-$row["RewOrReqMoney"]%10000)>=100?floor((-$row["RewOrReqMoney"]%10000)/100)."s":"")
							 .((-$row["RewOrReqMoney"]%100)>0?((-$row["RewOrReqMoney"]%100)."c"):"")
							 ."</td></tr>";
      if($row["RepObjectiveFaction"]>0 && $row["RepObjectiveValue"]>0)$temp.="<tr><td><b>Gain Reputation:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=".$row["RepObjectiveFaction"]),0)." => ".$row["RepObjectiveValue"]."</td></tr>";
      if($row["PlayersSlain"]>0)$temp.="<tr><td>Slay <b>".$row["PlayersSlain"]."</b> Enemy Players</td></tr>";
	...
	if(!empty($row["RewRepFaction".$i]) && (!empty($row["RewRepValue".$i]) || !empty($row["RewRepValueId".$i]) ))
	  {
	  $temp.="<tr><td>".mysql_result(mysql_query("SELECT name FROM $udbviewdb.faction WHERE id=".$row["RewRepFaction".$i]),0).": ";
...
$temp .= ($row["RewRepValueId".$i]?"<b>ID:</b> ".$row["RewRepValueId".$i]:"")."</td></tr>";
	...
	echo "</table></fieldset></td>";
      ...
      //Reward Other
      $temp="";
      if($row["RewMoneyMaxLevel"]>0)$temp.="<tr><td><b>Money at max Level/XP: Raw:</b> ".$row["RewMoneyMaxLevel"]."</td></tr>";
      if($row["RewXPId"]>0)$temp.="<tr><td><b>RewXPId: Raw:</b> ".$row["RewXPId"]."</td></tr>";
      if($row["BonusTalents"]>0)$temp.="<tr><td><b>Talent points:</b> ".$row["BonusTalents"]."</td></tr>";
      if($row["RewOrReqMoney"]>0)$temp.="<tr><td><b>Money:</b> ".($row["RewOrReqMoney"]>=10000?floor($row["RewOrReqMoney"]/10000)."g":"")
							 .(($row["RewOrReqMoney"]%10000)>=100?floor(($row["RewOrReqMoney"]%10000)/100)."s":"")
							 .(($row["RewOrReqMoney"]%100)>0?(($row["RewOrReqMoney"]%100)."c"):"")
							 ."</td></tr>";
      if($row["RewSpellCast"]>0 or $row["RewSpell"]>0)
	$temp.="<tr><td><b>Spell:</b> ".($row["RewSpell"]>0?mysql_result(mysql_query("SELECT name FROM $udbviewdb.spell WHERE id=".$row["RewSpell"]),0):"").($row["RewSpellCast"]>0 && $row["RewSpell"]>0?" -> ":"").($row["RewSpellCast"]>0?"Cast: ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.spell WHERE id=".$row["RewSpellCast"]),0):"")."</td></tr>";
      if($row["CharTitleId"]>0)$temp.="<tr><td><b>Title:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.chartitles WHERE id=".$row["CharTitleId"]),0)."</td></tr>";
      if($row["RewMailTemplateId"]>0)$temp.="<tr><td><b>Item by Mail:</b> ".mysql_result(mysql_query("SELECT name FROM $mangosdb.item_template WHERE entry=(SELECT item FROM $mangosdb.quest_mail_loot_template WHERE entry=".$quest.")"),0)." after ".floor($row["RewMailDelaySecs"]/60).":".($row["RewMailDelaySecs"]%60<10?"0":"").($row["RewMailDelaySecs"]%60)."</td></tr>";
      ...
      for($i=1;$i<=4;$i++)
          ...
          $emote_delay=$row["DetailsEmoteDelay".$i];
      	  $temp.="<tr><td>".mysql_result(mysql_query("SELECT name FROM $udbviewdb.emotes WHERE id=".$row["DetailsEmote".$i]),0)."(".$row["DetailsEmote".$i].") after ".$row["DetailsEmoteDelay".$i]."</td></tr>";
      	  }
      ...
      for($i=1;$i<=4;$i++)
      	...
      	if(!empty($row["OfferRewardEmote".$i]))
      	  ...
      	  $temp.="<tr><td>".mysql_result(mysql_query("SELECT name FROM $udbviewdb.emotes WHERE id=".$row["OfferRewardEmote".$i]),0)."(".$row["OfferRewardEmote".$i].") after ".$row["OfferRewardEmoteDelay".$i]."</td></tr>";
      	  }
      ...
      $temp="";
      	if(!empty($row["IncompleteEmote"]))
      	  {
      	  $temp.="<tr><td><b>Incomplete Emote:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.emotes WHERE id=".$row["IncompleteEmote"]),0)."(".$row["IncompleteEmote"].")</td></tr>";
      	  }
      	if(!empty($row["CompleteEmote"]))
      	  {
      	  $temp.="<tr><td><b>Complete Emote:</b> ".mysql_result(mysql_query("SELECT name FROM $udbviewdb.emotes WHERE id=".$row["CompleteEmote"]),0)."(".$row["CompleteEmote"].")</td></tr>";
      	  }
	...
	echo "<tr><td colspan=3><b>File a new Report</b></br><form action=index.php?showrev=".$show_data_for_rev."&filterstatus=".$filter_status."&quest=$quest&doreport method=post>
	      ...
	      for($i=0;$i<count($udbversion);$i++)
		echo "<option value=$i>$udbversion[$i]</option>";
